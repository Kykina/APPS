...
int8_t g_avg_temp = 0;
uint32_t g_flags = 0;
uint32_t g_flags_v2 = 0x000C8808;
...
int main() {
  ...
  printf("\n=== 2) Funkce pro výpočet (A - vnitřní) průměrné teploty za 16 dní ===\n");
  ...
  printf("\n=== 3-A) Funkce pro detekci mrazivých dní ===\n");

  check_low_temp();  // ASM funkce, která projde teploty, 
                     // pokud je počet teplota záporna, nastaví odpovídající bit na 1. 
